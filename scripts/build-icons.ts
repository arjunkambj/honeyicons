...
async function main() {
	...
	for (const icon of icons) {
		...
		for (const variant of icon.variants) {
			...
			variantNodes.push(serializeNodes(parseSvg(svg, relative)));
		}
		// Brand marks often use the same drawing in every variant; emit it once.
		const [firstNodes] = variantNodes;
		const shared =
			variantNodes.length > 1 &&
			variantNodes.every((nodes) => nodes === firstNodes);
		const nodeEntries = icon.variants.map(
			(variant, index) =>
				`\t${variant}: ${shared ? "nodes" : variantNodes[index]}`,
		);

		const source = `/* Generated by scripts/build-icons.ts. Do not edit. */
import { createIcon } from "../create-icon.js";
${shared ? `import type { IconNode } from "../types.js";\n\nconst nodes: IconNode = ${firstNodes?.replaceAll("\n\t", "\n")};\n` : ""}
