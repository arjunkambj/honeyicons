...
- Use a standard stroke weight of 1.8 units on the 24-unit grid, matching the renderer default. With normal scaling, this renders at 1.2px at 16px and 1.8px at 24px. Do not introduce per-icon stroke weights or non-scaling strokes by default.
...
- Use Solar-style smooth corner curves as the visual reference for new icons and requested rounding changes, including imports from Hugeicons. Match the actual path curvature; round stroke joins alone do not create Solar's corner shape. Preserve the icon's recognizable silhouette and proportions.
